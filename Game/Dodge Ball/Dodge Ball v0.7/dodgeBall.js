...
function onTouchMove(touchData){
	...
	if (timeElapsed > allTextDisplayTime) {
		...
		if (((playerBall.y + (currentmouseY - lastmouseY) / distScalar) >= 0 + playerBallRadius) &&
			 ((playerBall.y + (currentmouseY - lastmouseY) / distScalar) <= ySize - playerBallRadius)){
			playerBall.y += (currentmouseY - lastmouseY) / distScalar;
		};
	};
